...
func main() {
	...
	p := tpm2.PolicyAuthValue{
		PolicySession: sess.Handle(),
	}

	commandParameter, err := util.CPBytes(p)
	if err != nil {
	...
	log.Printf("PolicySecret commandParameter: %s\n", hex.EncodeToString(commandParameter))
	...
	sess2, cleanup2, err := tpm2.PolicySession(rwr, tpm2.TPMAlgSHA256, 16, []tpm2.AuthOption{tpm2.Auth([]byte("bar"))}...)
	if err != nil {
		log.Fatalf("setting up policy session: %v", err)
	}
	defer cleanup2()

	tp2 := tpm2.PolicyAuthValue{
		PolicySession: sess2.Handle(),
	}

	pa, err := util.CPBytes(tp2)
	if err != nil {
	...
	sess3, cleanup3, err := tpm2.PolicySession(rwr, tpm2.TPMAlgSHA256, 16, []tpm2.AuthOption{tpm2.Auth([]byte("bar"))}...)
	if err != nil {
		log.Fatalf("setting up policy session: %v", err)
	}
	defer cleanup3()
